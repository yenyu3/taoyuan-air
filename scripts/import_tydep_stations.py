...
# default JSON_DIR can be overridden by CLI `--json-dir`
...
def main():
    parser = argparse.ArgumentParser(description='TYDEP JSON import')
    parser.add_argument('--json-dir', type=Path, default=JSON_DIR,
                        help='TYDEP JSON 根目錄（預設 data/raw/tydep-stations/json）')
    ...
    if not args.json_dir.exists():
        print(f"[ERROR] 找不到 TYDEP JSON 目錄：{args.json_dir}")
        sys.exit(1)
    ...
    station_dirs = sorted([d for d in args.json_dir.iterdir() if d.is_dir()])
    if not station_dirs:
